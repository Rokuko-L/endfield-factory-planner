...
/**
 * Extra A* cost for stepping onto a tile that already carries a
 * connection. Crossings are legal (over/under pass) but expensive, so
 * the router only crosses when routing around is worse.
 */
const CROSSING_COST = 6;

/**
 * Multi-source / multi-target A* that allows crossing existing connections
 * perpendicular (auto-bridging). Machines block the path; connection tiles
 * are passable at a premium. Callers must validate that every shared tile
 * is a perpendicular crossing before accepting the path.
 */
...
function aStarPathCrossing(
  ...
  while (!open.isEmpty()) {
    ...
    if (current.x === end.x && current.y === end.y) {
      // No smoothing: it only walks fully-free lines and would erase crossings.
      return expandPath(reconstruct(current));
    }
    ...
    for (const { dx, dy } of DIRS) {
      ...
      // Crossing an existing line is legal but expensive — a last resort.
      const nk = key(nx, ny);
      if (closed.has(nk)) continue;

      const stepCost = grid.isFree(nx, ny) ? 1 : 1 + CROSSING_COST;
